...
public class Parser {
	...
	public Parser(String i, JTextArea o) throws IOException {
		lex =  new Lexer(i); out = o; move(); 
	}

	void move() throws IOException { look = lex.scan(); } // сканування чергового символа

	void error(String s) { 
		JOptionPane.showMessageDialog(null, "в рядку "+Lexer.line+": "+s);
		err = true; 
	} 
	...
	void match(int t) throws IOException { // перевірка чи сканований символ - t
		...
		else error("синтаксична помилка"); // якщо ні, вивід про помилку
   }
	
	public void program() throws IOException {  
		block(); // program -> block
		out.setText(dataseg+codeseg+"\n\tMOV AX,4C00h\n\tINT 21h\nEND Main\nEND"); // вивід асемблерного коду
	}
	...
	void rightConstr() throws IOException {
		Id id = top.get(look);
		  if( id == null ) error(look.toString() + " не оголошена");
			if ( id.type.equals("const") ) {
				  error(look.toString() + " константа, ви не можете змінити її");
		  }
		  match(Tag.ID);
		  if ( look.tag == Tag.INC ) {
	...
	String leftArithmeticExpr(Id id, String op1) throws IOException {
		...
		if ( look.tag == Tag.NUM || look.tag == Tag.ID ) {
			if (look.tag == Tag.ID ) { 
				if( lookId == null ) error(look.toString() + " не оголошена");
				if(lookId.type.equals("const")) constant=true;
			}
			op1 = look.toString(); move();
		} else if ( look.tag == Tag.BASIC ) {
			error("синтаксична помилка. \nОголошувати змінні в цьому місці не можна!\n Ти мені набрид, прощавай, невдаха!");
			System.exit(0);
		} else error("синтаксична помилка. Очікується число, змінна або константа");
		if ( look.tag == Tag.INC ) {
			if(constant) error(lookId.toString() + " константа. Її не можна змінювати");
	...
	void rightArithmeticExpr(Id id, String op, String op1, String op2, boolean bracket) throws IOException {
		while( look.tag == '+' || look.tag == '-' ) {
			...
			if ( look.tag == Tag.NUM || look.tag == Tag.ID ) {
				...
				error("синтаксична помилка. Не можна оголошувати змінні в цьому місці!");
			} else error("синтаксична помилка. Очікується дужка, число, змінна або константа"); 
			if ( look.tag == Tag.INC ) {
				if(constant) error(lookId.toString() + " константа. Її не можна змінювати!");
				op1 = id.lexeme; codeseg += "INC "+ op1 + "\n\t"; move();
			}  
	...
	void compareExpr() throws IOException {
		...
		switch( look.tag ) {
			...
			case Tag.EQ:
				...
				move(); break;
			default:
				error("синтаксична помилка. Очікується операція порівняння");
		}
		if ( look.tag == Tag.NUM || look.tag == Tag.ID ) {
			if (look.tag == Tag.ID ) {
				Id lookId = top.get(look);
